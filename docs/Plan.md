...
валидными данными на http://localhost:8080/api/v1/pay
Ожидаемый результат: статус 200, в БД появляется запись, статус карты APPROVED

2.Отправка POST запроса платежа с карты со статусом "DECLINED" и телом запроса с полностью заполненными
валидными данными на http://localhost:8080/api/v1/pay
Ожидаемый результат: статус 200, статус карты DECLINED

3. Отправка POST запроса платежа в кредит с карты со статусом "APPROVED", и телом запроса с полностью заполненными
валидными данными на http://localhost:8080/api/v1/credit
Ожидаемый результат: статус 200, появление соответствующей записей в БД, статус карты APPROVED
