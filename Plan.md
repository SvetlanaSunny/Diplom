...
- Карта, с валидными данными со статусом "APPROVED" - одобрена
- Карта, с валидными данными со статусом "Blocked" - заблокирована

###UI тесты
1. Открыть в браузере страницу по адресу localhost:8080
_**Ожидаемый результат:** "Открывается главная страница веб-сервис, который предлагает купить тур 
по определённой цене двумя способами"_

2. Нажать на главной странице на кнопку "Купить"
Ожидаемый результат: На главной странице веб-сервиса становиться видимой/доступной
операция "Оплата по карте"

3. Заявка "Оплата по карте", заполняется валидными данными карты со статусом APPROVED
...
4. Заявка "Оплата по карте", заполняется валидными данными карты со статусом Blocked
- все поля заполняются валидными данными, номер карты - валидными данными со статусом "Blocked"
...
9. Заявка "Олата по карте" поле номер месяца заполнено невалидным числом (больше 12)
-все поля заполняются валидными данными, поле месяц заполнено числом больше "12"
Ожидаемый результат: под полем месяц появляется сообщение об ошибке "Неверно указан срок действия карты"

10. Заявка "Олата по карте" поле номер месяца заполнен месяцем меньше текущего, будущего года
...
11. Заявка "Олата по карте" поле номер месяца заполнен месяцем меньше текущего ,текущего года
...
12. Заявка "Олата по карте" поле номер месяца не заполнено
...
13. Заявка "Олата по карте" поле год не заполнено
...
14. Заявка "Олата по карте" поле год заполнено двумя нулями 
...
15. Заявка "Олата по карте" поле год заполнено годом ранее текущего
-все поля заполняются валидными данными, поле год заполнено годом предыдущем от текущего
Ожидаемый результат: под полем год появляется сообщение об ошибке "Истёк срок действия карты"

16. Неверно указан срок действия карты
...
17. Заявка "Олата по карте" поле владелец заполнено кирриллицей
...
18. Заявка "Олата по карте" поле владелец не заполнено
...
###Все тесты начиная с 3 по 26, провести на функционале "Кредит по данным карты"
...
3.Отправка POST запроса платежа с карты со статусом "Blocked"  и телом запроса с полностью заполненными
валидными данными на http://localhost:8080/payment
Ожидаемый результат: статус 400, в БД не появляется запись 

4.Отправка POST запроса платежа в кредит с карты со статусом "Blocked"  и телом запроса с полностью заполненными
валидными данными на http://localhost:8080/credit
Ожидаемый результат: статус 400, в БД не появляется запись
